...
import com.margoni.surfingspots.data.network.client.CitiesApi
...
class CityDataSourceImpl(
    ...
    override suspend fun list(): List<City> {
        return api.fetch().cities.map { City(it.name, imageFor(it.name)) }
    }

    private fun imageFor(city: String): String {
